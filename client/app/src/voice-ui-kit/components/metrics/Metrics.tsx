...
export const Metrics: React.FC<Props> = ({
  ...
  useRTVIClientEvent(RTVIEvent.Metrics, (data) => {
    ...
    if (data?.ttfb && Array.isArray(data.ttfb)) {
      ...
      setTtfbMetrics((prevMetrics) => {
        ...
        ;(data.ttfb ?? []).forEach((item: ProcessingMetric) => {
          ...
          const updatedMetrics = [...newMetrics[processor], { timestamp, value }].slice(-100)
    ...
    if (tokens && Array.isArray(tokens) && tokens.length > 0) {
      ...
      setTokenMetrics((prev) => ({
        completion_tokens:
          prev.completion_tokens + (noCompletionTokens ? 0 : tokenData.completion_tokens || 0),
  ...
  const tokenCardClassName = cn("bg-card rounded-md p-3 shadow-sm", classNames.tokenCard)
  const tokenTypeClassName = cn("text-sm text-muted-foreground", classNames.tokenType)
  ...
  if (hasMetrics || hasTokenMetrics) {
    return (
      ...
      >
        {hasTokenMetrics && (
          <>
            <h2 className={cn("text-xl font-semibold", classNames.heading)}>Token Usage</h2>
            <div
              className={cn(
            ...
            >
              {!noPromptTokens && (
                <div className={tokenCardClassName}>
                  ...
                  <div className={tokenValueClassName}>{tokenMetrics.prompt_tokens}</div>
                </div>
              )}
              {!noCompletionTokens && (
                <div className={tokenCardClassName}>
                  <div className={tokenTypeClassName}>Completion Tokens</div>
                  <div className={tokenValueClassName}>{tokenMetrics.completion_tokens}</div>
                </div>
              )}
              {!noTotalTokens && (
                <div className={tokenCardClassName}>
                  <div className={tokenTypeClassName}>Total Tokens</div>
                  <div className={tokenValueClassName}>{tokenMetrics.total_tokens}</div>
                </div>
              )}
            </div>
          </>
        ...
        {hasMetrics && (
          <>
            <h2 className={cn("text-xl font-semibold", classNames.heading)}>TTFB Metrics</h2>
            <div
              className={cn(
                "grid grid-cols-1 @xl/metrics:grid-cols-2 @3xl/metrics:grid-cols-3 gap-4",
            ...
            >
              {Object.entries(ttfbMetrics).map(([processorName, data]) => (
                ...
                >
                  <h3 className={cn("mb-2", classNames.metricsTitle)}>{processorName}</h3>
    ...
    return (
      ...
      >
        <div className="p-4">
          ...
          <p className="text-sm max-w-md">Connect to an agent to view metrics in real-time.</p>
        </div>
      </div>
    )
  }
