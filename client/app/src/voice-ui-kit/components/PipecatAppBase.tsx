...
import { ThemeProvider, type ThemeProviderProps } from "@/components/ThemeProvider"
...
const defaultStartBotResponseTransformer = (response: TransportConnectionParams) => response
...
export const PipecatAppBase: React.FC<PipecatBaseProps> = ({
  ...
  >(null)
  const [transformedStartBotResponse, setTransformedStartBotResponse] = useState<
    TransportConnectionParams | unknown
  >(null)
        ...
        if (startBotParams) {
          ...
          if (transportType === "smallwebrtc") {
            ...
            if (typeof response === "object" && response !== null && "iceConfig" in response) {
              ...
              ;(client.transport as SmallWebRTCTransport).iceServers = iceConfig.iceServers
            }
          }
          const transformedResponse = await startBotResponseTransformer(response)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const transport = await createTransport(transportType, transportOptions)
   ...
   */
  if (!client) {
    return typeof children === "function" ? children({ client: null, error: null }) : children
  }
  ...
  return noThemeProvider ? clientProvider : (
