...
const PanelHeaderVariants = cva("@container/panel-header", {
  variants: {
    variant: {
      ...
      noPadding: "border-b flex items-center justify-center text-card-foreground",
    },
  },
  defaultVariants: {
    variant: "default",
...
function PanelTitle({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="panel-title" className={cn("mono-upper", className)} {...props} />
}
...
function PanelFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex items-center p-2 @xs/panel:p-3 @md/panel:p-4", className)}
      {...props}
    />
  )
}
...
export { Panel, PanelActions, PanelContent, PanelFooter, PanelHeader, PanelTitle }
