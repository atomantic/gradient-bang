...
export const BotAudioPanel: React.FC<BotAudioPanelProps> = ({ className, collapsed = false }) => {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        const barWidth = Math.max(Math.min(width / (barCount * 2), maxBarWidth), 2)
      ...
      >
        <div ref={containerRef} className="relative flex h-full overflow-hidden">
          ...
          : <div className="text-subtle flex w-full gap-2 items-center justify-center">
              <MicOffIcon size={16} />
              {!collapsed && <span className="font-semibold text-sm">No audio</span>}
            </div>
          }
        </div>
      </PanelContent>
