...
import { ChevronDownIcon, MicIcon, MicOffIcon, SpeakerIcon } from "lucide-react"
...
export const UserAudioComponent: React.FC<ComponentProps> = ({
  ...
  if (noAudio || buttonProps?.isLoading) {
    buttonComp = (
      <Button
        ...
        {...buttonProps}
        disabled
        className={cn(btnClasses, buttonProps?.isLoading && "justify-center", classNames.button)}
      >
    ...
    const hasMicrophones = !noMicrophones && availableMics && availableMics.length > 0
    const hasSpeakers = !noSpeakers && availableSpeakers && availableSpeakers.length > 0
    ...
    buttonComp = (
      <>
        <Button
          ...
          size={size}
          {...buttonProps}
          className={cn(btnClasses, !noDropdown && "rounded-e-none", classNames.button)}
        >
          {!noIcon && (isMicEnabled ? <MicIcon /> : <MicOffIcon />)}
          {buttonState === "inactive" && inactiveText ?
            <span className={cn("flex-1", classNames.inactiveText)}>{inactiveText}</span>
          : null}
          {buttonState !== "inactive" && activeText ?
            <span className={cn("flex-1 ", classNames.activeText)}>{activeText}</span>
          : null}
          {children}
          {!noVisualizer && (
            <VoiceVisualizer
  ...
  return (
    <ButtonGroup className={cn(variant !== "outline" && "gap-[1px]")}>{buttonComp}</ButtonGroup>
  )
}
 ...
 * ```
 */
export const UserAudioControl: React.FC<Props> = ({ buttonProps, ...props }) => {
  const {
    availableMics,
    selectedMic,
    updateMic,
  ...
  const loading = transportState === "disconnected" || transportState === "initializing"
