...
export interface DeviceSelectBaseProps extends Omit<SelectTriggerProps, "children"> {
  placeholder?: string
...
export const DeviceSelectComponent = ({
  ...
  return (
    <Select value={selectedValue} onValueChange={(v) => updateDevice?.(v)} {...selectProps}>
      <SelectTrigger className={cn(classNames?.selectTrigger, className)} {...triggerProps}>
        {guide ?
          <SelectGuide>{guide}</SelectGuide>
        : null}
        <SelectValue placeholder={placeholder} />
      </SelectTrigger>
      <SelectContent className={cn(classNames?.selectContent)} {...contentProps}>
