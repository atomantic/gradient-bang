...
export const SessionInfo: React.FC<Props> = ({
  ...
  if (client && "dailyCallClient" in client.transport) {
    ...
    client?.transport.__proto__.constructor.SERVICE_NAME === "small-webrtc-transport"
  ) {
  ...
  if (!noRTVIVersion) {
    ...
    data["RTVI Client"] = clientVersion ? `v${clientVersion}` : <TextDashBlankslate />
    data["RTVI Server"] = serverVersion ? `v${serverVersion}` : <TextDashBlankslate />
  }

  return <DataList classNames={{ container: "w-full overflow-hidden" }} data={data} />
}
