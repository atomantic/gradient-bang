...
import { ChevronDownIcon, LoaderCircle, VideoIcon, VideoOffIcon } from "lucide-react"
...
export const UserVideoComponent: React.FC<UserVideoComponentProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <ButtonGroup className={cn(variant !== "outline" && "gap-[1px]", classNames.buttongroup)}>
          <Button
            className={cn(
              {
                "w-fit! hover:opacity-100! hover:bg-muted hover:text-muted-foreground": !noVideo,
          ...
          {!noDevicePicker && (
            ...
            >
              <Button
                className={cn(
                  {
                    "hover:opacity-100! hover:bg-primary hover:text-primary-foreground": !noVideo,
...
export const UserVideoControl: React.FC<UserVideoControlBaseProps> = ({
  ...
  const { availableCams, selectedCam, updateCam } = usePipecatClientMediaDevices()
