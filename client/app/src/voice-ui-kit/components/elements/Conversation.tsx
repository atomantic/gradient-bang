...
export const Conversation: React.FC<ConversationProps> = memo(
  ({
    ...
    if (messages.length > 0) {
      return (
        <div className={cn("relative h-full flex flex-col", classNames.container)}>
          <div
            ref={scrollRef}
            className={cn("relative flex-1 overflow-y-auto p-4", !noTextInput && "pb-2")}
          >
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center p-4 max-w-md">
              ...
              <p className="text-sm text-muted-foreground">
                This server does not support BotOutput events (requires RTVI 1.1.0+). Conversation
                messages cannot be displayed without BotOutput event support.
              </p>
            </div>
          </div>
          {textInput}
    ...
    return (
      <div className={cn("relative h-full flex flex-col", classNames.container)}>
        <div className="flex-1 flex items-center justify-center">
          <div className="text-muted-foreground text-sm">Waiting for messages...</div>
        </div>
        {textInput}
      </div>
    )
