...
export function usePipecatEventStream(options?: UsePipecatEventStreamOptions) {
  ...
  const allEventNames = useMemo(() => {
    ...
    return (Object.values(RTVIEvent).filter((v) => typeof v === "string") as string[]).slice()
  }, [])
  ...
  const handleRTVIEvent = useCallback(
    ...
    [ignoredEventsSet, includedEventsSet, maxEvents, scheduleNotify, mapEventData, onEvent]
  )
  ...
  useEffect(() => {
    ...
    return () => {
      for (const [eventName, handler] of Object.entries(handlers)) {
        try {
          client.off(eventName as ClientEventName, handler as Parameters<ClientInstance["off"]>[1])
        } catch {
          // ignore
        }
      }
    }
  }, [client, handleRTVIEvent, allEventNames, ignoredEventsSet, includedEventsSet])
