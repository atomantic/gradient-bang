 ...
 */
export type PipecatConnectionState = "disconnected" | "connecting" | "connected"
...
const deriveState = (state: TransportState): PipecatConnectionState => {
  if (state === "ready") {
    return "connected"
  } else if (["authenticating", "authenticated", "connecting", "connected"].includes(state)) {
    return "connecting"
  } else {
    return "disconnected"
  }
...
export const usePipecatConnectionState = (): PipecatConnectionStateResult => {
  ...
  const [connectionState, setConnectionState] = useState<PipecatConnectionState>(
