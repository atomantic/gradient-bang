  ...
  forwardRef<PlasmaRef, PlasmaProps>(
    (
      ...
      const targetRef = useRef<
        Record<string, number | boolean | { x: number; y: number } | THREE.Vector3>
      >({})
      ...
      const applyAudioReactivity = useCallback(() => {
        ...
        const compressedVolume = volume < threshold ? 0 : (volume - threshold) / (1 - threshold) // Compress remaining range to 0-1
        ...
        setShaderValue("intensity", configRef.current.intensity * (1 + volumeResponse * 1), true)
        setShaderValue("radius", configRef.current.radius * (1 + volumeResponse * 0.6), true)
        setShaderValue(
          "effectScale",
        ...
        if (volume >= threshold) {
          ...
          setShaderValue("ringVisibility", configRef.current.ringVisibility, false)
          setShaderValue("ringThickness", configRef.current.ringThickness, false)
          setShaderValue("ringDistance", configRef.current.ringDistance, false)
          setShaderValue("ringAmplitude", configRef.current.ringAmplitude, false)
      ...
      const updateInterpolation = useCallback(() => {
        ...
        if (targetRef.current.effectCenter && currentRef.current.effectCenter) {
          ...
          if (isVector(current) && isVector(target)) {
            ...
            materialRef.current!.uniforms.effectCenter.value.set(current.x, current.y)
          }
        }

        // Update colors
        ;["color1", "color2", "color3", "backgroundColor"].forEach((colorKey) => {
          if (targetRef.current[colorKey] && currentRef.current[colorKey]) {
            ...
            if (target instanceof THREE.Vector3 && current instanceof THREE.Vector3) {
              current.x = lerp(current.x, target.x, lerpSpeed)
              current.y = lerp(current.y, target.y, lerpSpeed)
              current.z = lerp(current.z, target.z, lerpSpeed)
              materialRef.current!.uniforms[colorKey].value.set(current.x, current.y, current.z)
            }
          }
        })
      }, [])
        ...
        Object.entries(newConfig).forEach(([key, value]) => {
          if (value !== undefined) {
            ...
            if (key === "effectCenter" && typeof value === "object") {
              ...
              targetRef.current[key] = value as number | boolean | { x: number; y: number }
            }

            // Update immediate properties directly
            if (immediateProps.includes(key) && materialRef.current?.uniforms[key]) {
              materialRef.current.uniforms[key].value = value
            }

            // Update non-interpolated properties
            if (key === "useCustomColors" && materialRef.current?.uniforms.useCustomColors) {
              materialRef.current.uniforms.useCustomColors.value = value as boolean
            }
            if (key === "lerpSpeed") {
      ...
      useEffect(() => {
        ...
        ;["color1", "color2", "color3", "backgroundColor"].forEach((colorKey) => {
          const hex = configRef.current[colorKey as keyof PlasmaConfig] as string
        ...
        const uniforms = {
          ...
          color1: {
            value:
              isVector3(currentRef.current.color1) ?
                currentRef.current.color1.clone()
              : new THREE.Vector3(),
          },
          color2: {
            value:
              isVector3(currentRef.current.color2) ?
                currentRef.current.color2.clone()
              : new THREE.Vector3(),
          },
          color3: {
            value:
              isVector3(currentRef.current.color3) ?
          ...
          backgroundColor: {
            value:
              isVector3(currentRef.current.backgroundColor) ?
