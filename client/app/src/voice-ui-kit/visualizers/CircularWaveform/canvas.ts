...
export class CircularWaveformCanvas {
  ...
  private generateConfig(): WaveformConfig {
    const { sensitivity, numBars, barWidth, rotationSpeed, rotationBoostFactor } = this.options
  ...
  private initializeAudio(): AudioContext | null {
    ...
    try {
      ...
      const downsampledLength = Math.ceil(analyser.frequencyBinCount / this.config.downsampleFactor)
  ...
  private processAudioData(frequencyData: Uint8Array): void {
    ...
    for (let i = 0; i < this.downsampledData.length; i++) {
      ...
      // Track maximum value
      if (i >= vocalLowBin && i <= vocalHighBin && this.normalizedData[i] > maxValue) {
    ...
    this.calculateOptimizedAudioLevels(frequencyData, vocalLowBin, vocalHighBin)
  }
  ...
  private applyOptimizedTemporalSmoothing(): void {
    if (!this.prevFrameData || this.prevFrameData.length !== this.smoothedData!.length) {
    ...
    for (let i = 0; i < this.smoothedData!.length; i++) {
      this.temporallySmoothedData[i] =
        this.smoothedData![i] * (1 - smoothFactor) + this.prevFrameData[i] * smoothFactor
  ...
  private calculateOptimizedAudioLevels(
    ...
    const bassLevel = Math.max(0.2, (bassSum / (bassCount || 1) / 255) * this.config.bassBoost)
    ...
    if (this.options.rotationEnabled) {
      if (isActiveAudio) {
        ...
        const boostAmount = Math.min(1.0, overallActivityLevel) * this.config.rotationBoostFactor
        this.targetRotationSpeed = this.config.rotationSpeed * (1 + boostAmount)
      } else {
    ...
    for (let i = 0; i < barCount; i++) {
      ...
      if (normalizedPosition < 0.8) {
        ...
        const frequencyIndex = Math.floor(
          speechRangeLow + (normalizedPosition * (speechRangeHigh - speechRangeLow)) / 0.8
        )
  ...
  private applyEnhancedFrequencySpread(values: number[]): void {
    ...
    for (let i = 0; i < len; i++) {
      if (temp[i] > 0.08) {
        ...
        const isInLowActivityZone = i >= Math.floor(len * 0.6) && i <= Math.floor(len * 0.9)

        // Use a wider spread and stronger influence in low activity zones
        const effectiveSpreadRadius = isInLowActivityZone ? spreadRadius * 1.5 : spreadRadius
        ...
        for (let offset = 1; offset <= effectiveSpreadRadius; offset++) {
          ...
          const targetIsInLowZone =
            (leftIdx >= Math.floor(len * 0.6) && leftIdx <= Math.floor(len * 0.9)) ||
            (rightIdx >= Math.floor(len * 0.6) && rightIdx <= Math.floor(len * 0.9))
          ...
          values[rightIdx] = Math.max(values[rightIdx], values[rightIdx] * 0.6 + spreadValue * 0.4)
        }
      }
    }
    ...
    for (let pass = 0; pass < smoothingPasses; pass++) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const isInTransitionZone = i >= Math.floor(len * 0.55) && i <= Math.floor(len * 0.95)
        ...
        if (isInTransitionZone || pass === 0) {
          ...
          values[i] =
            (resultCopy[prev] + resultCopy[i] * centerWeight + resultCopy[next]) /
            (2 + centerWeight)
        }
      }
    }
    ...
    if (maxVal > minVal * 3 && minVal > 0) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const boostedVal = Math.sqrt(normalizedVal) * range * 0.8 + minVal * 1.2
  ...
  private processBarValueOptimized(
    ...
    if (isActiveAudio) {
      const oscillationIntensity = Math.min(0.04, Math.max(0.01, overallActivityLevel * 0.12))

      const oscillation =
        Math.sin(this.animationTime * 1.5 + position * Math.PI * 2) * oscillationIntensity
      ...
      const restingOscillation = Math.sin(this.animationTime * 0.8 + position * Math.PI * 2) * 0.002
    ...
    if (this.options.state === WaveformState.AUDIO) {
      const audioPulseInfluence = this.getAudioPulseInfluence(barIndex, barCount)
      ...
      if (audioPulseInfluence > 0) {
        ...
        value = value * (1 + thinkingPulseInfluence * 0.1) + thinkingPulseInfluence * 0.01
      }
    }
  ...
  private draw = (timestamp: number) => {
    ...
    const deltaTime = Math.min(Math.max((timestamp - this.lastFrameTime) / 1000, 0.001), 0.1)
  ...
  private drawThinkingState(
    ...
    const pulsePosition = (this.animationTime * this.config.pulseSpeed) % (Math.PI * 2)
    ...
    for (let i = 0; i < barCount; i++) {
      ...
      const pulseInfluence = Math.max(0, 1 - angleDiff / (this.config.pulseWidth / 2))

      // Create a smooth pulse shape with reduced amplitude
      const smoothedPulse = Math.pow(Math.sin((pulseInfluence * Math.PI) / 2), 2) * pulseHeight
      ...
      const newBaseValue =
        bar.targetValue * (1 - stabilizationFactor) + baseSize * stabilizationFactor
      ...
      const smoothingFactor =
        bar.targetValue > bar.value ? Math.min(1, growFactor) : Math.min(1, shrinkFactor)
  ...
  private drawStaticState(
    ...
    for (let i = 0; i < this.freqBars.length; i++) {
      ...
      const randomValue = baseSize + Math.sin(this.animationTime + i * 0.1) * variationAmount
  ...
  private drawAudioState(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement): void {
    // Use logical dimensions
    const width = canvas.width / this.dpr
    const height = canvas.height / this.dpr
    const centerX = width / 2
  ...
  private drawBar(
    ...
    const color = this.blendColors(this.options.color1, this.options.color2, colorBlendRatio)
    ...
    const thicknessRatio =
      minWidthRatio + (maxWidthRatio - minWidthRatio) * Math.pow(normalizedValue, 2.2)
  ...
  private blendColors(color1: string, color2: string, ratio: number): string {
    ...
    const a = c1.a !== undefined && c2.a !== undefined ? c1.a * (1 - ratio) + c2.a * ratio : 1
  ...
  public updateOptions(options: Partial<CanvasWaveformOptions>): void {
    ...
    if (this.isPlaying && this.options.sensitivity !== prevSensitivity && this.audioContext) {
    ...
    switch (state) {
      ...
      case WaveformState.AUDIO:
        ...
        if (prevState === WaveformState.THINKING) {
          ...
          this.nextPulseTime = this.animationTime + this.getRandomPulseInterval() + 1.0
        } else {
  ...
  private updateRotationSpeed(deltaTime: number): void {
    ...
    if (this.options.state === WaveformState.AUDIO && this.analyser && this.frequencyData) {
      ...
      if (hasVoiceActivity) {
        ...
        this.targetRotationSpeed = this.config.rotationSpeed * (1 + boostFactor)
        ...
        if (speedDiff > 0) {
          this.currentRotationSpeed += speedDiff * Math.min(1, accelerationRate * deltaTime)
        }
      } else {
        ...
        if (speedDiff < 0) {
          this.currentRotationSpeed += speedDiff * Math.min(1, decelerationRate * deltaTime)
        }
      }
   ...
   */
  private limitBarValue(value: number, maxValue: number = this.config.maxBarLength): number {
   ...
   */
  private getThinkingPulseInfluence(barIndex: number, barCount: number): number {
    // Create a pulse that travels around the circle
    const normalizedPosition = barIndex / barCount
