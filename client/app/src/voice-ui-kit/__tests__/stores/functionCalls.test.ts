...
import { isMessageEmpty, mergeMessages, useConversationStore } from "@/stores/conversationStore"
...
describe("conversationStore – function calls", () => {
  ...
  describe("updateFunctionCall", () => {
    ...
    it("updates function_name if provided", () => {
      ...
      expect(getState().messages[0].functionCall!.function_name).toBe("get_weather")
    })
    ...
    it("updates the last matching function call when duplicates exist", () => {
      ...
      const messages = getState().messages
      const call1 = messages.find((m) => m.functionCall?.tool_call_id === "call_1")
      const call2 = messages.find((m) => m.functionCall?.tool_call_id === "call_2")
  ...
  describe("updateLastStartedFunctionCall", () => {
    ...
    it("targets the last started entry when multiple exist", () => {
      ...
      const messages = getState().messages
      const updated = messages.find((m) => m.functionCall?.tool_call_id === "call_b")
  ...
  describe("deduplication via addMessage pipeline", () => {
    it("keeps the most advanced status when duplicates share a tool_call_id", () => {
      ...
      const fcMessages = messages.filter((m) => m.functionCall?.tool_call_id === "call_1")
  ...
  describe("function call lifecycle", () => {
    it("transitions started → in_progress → completed", () => {
      ...
      let fc = getState().messages.find((m) => m.functionCall?.tool_call_id === "call_abc")
      ...
      fc = getState().messages.find((m) => m.functionCall?.tool_call_id === "call_abc")
      expect(fc).toBeDefined()
    ...
    it("handles InProgress arriving before Started", () => {
      ...
      const fc = messages.find((m) => m.functionCall?.tool_call_id === "call_1")
      expect(fc!.functionCall!.status).toBe("completed")
