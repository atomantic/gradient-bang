...
export function getRawPartTexts(message: ConversationMessage): string[] {
  return message.parts.filter((p) => typeof p.text === "string").map((p) => p.text as string)
}
...
export function getAssistantText(message: ConversationMessage): string {
  return message.parts
    .map((p) => {
      ...
      if (p.text && typeof p.text === "object" && "spoken" in p.text && "unspoken" in p.text) {
...
export function expectMessages(
  ...
  expected.forEach((exp, i) => {
    ...
    if (exp.textContains) {
      const text = msg.role === "assistant" ? getRawPartTexts(msg).join("") : getUserText(msg)
