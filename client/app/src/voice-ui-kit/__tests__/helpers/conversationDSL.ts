      ...
      if (turn.kind === "bot") {
        ...
        if (interrupt) {
          ...
          for (const word of allSpokenWords) {
            ...
            const targetNorm = interruptWords[matched].toLowerCase().replace(/[^\w\s]/g, "")
            steps.push({
              type: "botOutput",
              text: word,
              spoken: true,
        ...
        const toolCallId = turn.options.toolCallId ?? `call_${++this.functionCallCounter}`
 ...
 */
export function playScenario(harness: StoreHarness, scenario: ConversationScenario) {
  ...
  for (const step of scenario.steps) {
    switch (step.type) {
      ...
      case "botStoppedSpeaking": {
        ...
        const lastAssistant = messages.findLast((m: ConversationMessage) => m.role === "assistant")
      ...
      case "userStartedSpeaking": {
        ...
        const lastUserIdx = msgs.findLastIndex((m: ConversationMessage) => m.role === "user")
      ...
      case "userStoppedSpeaking": {
        ...
        userStoppedTimeout = setTimeout(() => {
          const msgs = harness.getMessages()
          const lastUser = msgs.findLast((m: ConversationMessage) => m.role === "user")
