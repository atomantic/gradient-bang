...
import { type APIRequest, type PipecatClientOptions, RTVIEvent } from "@pipecat-ai/client-js"
import { usePipecatClientCamControl, useRTVIClientEvent } from "@pipecat-ai/client-react"
import {
  BotIcon,
...
export interface ConsoleTemplateProps extends Omit<PipecatBaseProps, "children"> {
...
const getConnectionUrl = (
  ...
  if (startBotParams?.endpoint) return resolveUrl(String(startBotParams.endpoint))
  const url = extractUrlFromOptions(connectParams) ?? extractUrlFromOptions(transportOptions)
...
const defaultTransportOptions: React.ComponentProps<typeof PipecatAppBase>["transportOptions"] = {}
...
const ConsoleUI = ({
  ...
  const connectionUrl = getConnectionUrl(startBotParams, connectParams, transportOptions)
    ...
    <>
      ...
      <div className="flex flex-col h-full w-full overflow-auto">
        <div className="h-min grid grid-cols-2 sm:grid-cols-[150px_1fr_150px] gap-2 items-center justify-center p-2 bg-background sm:relative top-0 w-full z-10">
          {noLogo ?
            <span className="h-6" />
          : (logoComponent ?? <PipecatLogo className="h-6 w-auto text-foreground" />)}
              ...
              <ResizablePanelGroup direction="horizontal">
                {!noBotArea && (
                  <>
                    ...
                    {(!noConversationPanel || !noInfoPanel) && <ResizableHandle withHandle />}
                  </>
                )}
                {!noConversationPanel && (
                  <>
                      ...
                      <div className="flex flex-col items-center justify-center gap-4 h-full">
                        ...
                        {!noSessionInfo && (
                          <Popover>
                            ...
                            <PopoverContent side="left">
                              <SessionInfo participantId={participantId} sessionId={sessionId} />
                            </PopoverContent>
                          </Popover>
                        )}
                      </div>
