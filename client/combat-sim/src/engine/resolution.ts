...
// Ported verbatim from `deployment/supabase/functions/_shared/combat_engine.ts`.
// Keep this file dependency-free and Deno-compatible so it moves with the engine
// port unchanged.
...
export const SHIELD_REGEN_PER_ROUND = 10
...
export function resolveRound(
  ...
  function pickFleeOpponent(fleerId: string): CombatantState | null {
    ...
    candidates.sort((a, b) => {
      ...
      if (a.combatant_id < b.combatant_id) return -1
      if (a.combatant_id > b.combatant_id) return 1
      return 0
    })
  ...
  while (Object.values(remainingCommits).some((value) => value > 0)) {
    ...
    for (const pid of attackOrder) {
      if (remainingCommits[pid] <= 0) continue
      ...
      const probability = clamp(
        BASE_HIT - targetMitigation * MITIGATE_HIT_FACTOR + attackerMitigation * ATTACK_BONUS_FACTOR,
        MIN_HIT,
        MAX_HIT,
      )
  ...
  if (!livingNotFled.length) {
    ...
    const losers = participantIds.filter(
      (pid) => pid !== livingNotFled[0] && !fleeResults[pid] && fightersRemaining[pid] <= 0,
    )
    if (losers.length) {
      if (losers.length === 1) {
