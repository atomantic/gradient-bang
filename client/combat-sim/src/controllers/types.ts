...
export interface DecisionTrace {
  ...
  // Messages that were appended to the agent's context AFTER this decision —
  // the assistant tool-call turn, the tool-result message, any action_accepted
  // XML from the commit, and — if this decision was the final submit that
  // triggered round resolution — the round_resolved / round_waiting(next) /
  // combat.ended XML that landed inside the same sync commit chain. Rendered
  // as a separate "after the decision" section in the trace view so you can
  // see what this round's outcome WAS without opening the next round's trace.
