-- Quest evaluation: allow events with only actor_character_id (no character_id)
--
-- Corp-scoped events like corporation.member_joined have character_id = NULL
-- but can have actor_character_id set. The previous guard rejected these events
-- entirely, preventing corp join from triggering quest progress.
--
-- Fix: only bail out when BOTH character_id and actor_character_id are NULL.
...
BEGIN
  ...
  -- Corp-scoped events (e.g. corporation.member_joined) have character_id = NULL
  -- but actor_character_id identifies the acting player.
